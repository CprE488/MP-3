...
#define LAUNCHER_MINOR_BASE	192
...
#define to_launcher_dev(d) container_of(d, struct launcher, kref)

static struct usb_driver launcher_driver;
static void launcher_draw_down(struct launcher *dev);

static void launcher_delete(struct kref *kref)
{
	struct launcher *dev = to_launcher_dev(kref);
...
static int launcher_do_read_io(struct launcher *dev, size_t count)
{
	int rv;
...
static struct usb_class_driver launcher_class = {
	.name =		"launcher%d",
	.fops =		&launcher_fops,
	.minor_base =	LAUNCHER_MINOR_BASE,
};
...
{
	struct launcher *dev = usb_get_intfdata(intf);
